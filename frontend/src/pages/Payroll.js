...
function Payroll() {
  ...
  return (
    <div>
      ...
      >
        {/* Filters */}
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'nowrap', overflowX: 'auto', gap: 8 }}>
          <div>
        ...
        <ModalForm
          ...
          onSubmit={async (form, reset) => {
            ...
            setLoading(true);
            try {
              // Filter out empty deductions and convert amounts to numbers
              const validDeductions = deductions
                .filter(d => d.amount && d.reason)
                .map(d => ({
          ...
          renderFields={(form, handleChange) => {
            ...
            const netSalary = basic + hra + specialAllowance + bonuses - (pf + esi + professionalTax + incomeTax + totalOtherDeductions);
            ...
            return (
              <>
                ...
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontWeight: 600, marginBottom: 4 }}>Other Deductions (Optional)</div>
                  {deductions.map((ded, idx) => (
                    <div key={idx} style={{ display: 'flex', gap: 8, marginBottom: 4 }}>
                      ...
                      />
                      <button type="button" onClick={() => setDeductions(deductions.filter((_, i) => i !== idx))} style={{ background: '#eee', border: 'none', borderRadius: 4, padding: '0 8px', cursor: 'pointer', color: '#c00' }} disabled={deductions.length === 1}>-</button>
                    </div>
                  ))}
                  <button type="button" onClick={() => setDeductions([...deductions, { amount: '', reason: '' }])} 
                ...
                <input value={totalOtherDeductions} readOnly placeholder="Total Other Deductions (auto)" style={{ width: '100%', marginBottom: 10, padding: 8, borderRadius: 4, border: '1px solid #eee', background: '#f8fafd' }} />
